...
function updateFileToHuaweiObs(key: string, filePath: string, logger: Logger): Promise<any> {
    return new Promise((resolve, reject) => {
        const obsClient = new ObsClient({
            access_key_id: _.get(config, 'obs.accessKeyId'),
            secret_access_key: _.get(config, 'obs.secretAccessKey'),
            server : _.get(config, 'obs.endpoint'),
        });
        obsClient.putObject({
            Bucket : _.get(config, 'obs.bucketName'),
            Key : key,
            SourceFile : filePath,
        }, (err, result) => {
            if(err){
...
export function uploadFileToStorage(key: string, filePath: string, logger: Logger): Promise<void> {
    ...
    switch (storageType) {
        ...
        case 'obs':
            return updateFileToHuaweiObs(key, filePath, logger);
        default:
